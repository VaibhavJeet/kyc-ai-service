...
# Health check - use Python for reliability (no extra dependencies needed)
# Increased start-period to 90s to allow AI models to fully load
HEALTHCHECK --interval=30s --timeout=10s --start-period=90s --retries=3 \
    CMD python -c "import urllib.request, sys; sys.exit(0 if urllib.request.urlopen('http://localhost:8001/api/v1/health').getcode() == 200 else 1)"
